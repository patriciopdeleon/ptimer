...
tiles.forEach((el,i)=>{
  ...
  el.querySelectorAll('.inline-editor input').forEach(input=>{
    ...
    input.addEventListener('keydown',e=>{
      ...
      if(e.key==='Enter'){e.preventDefault();if(input.classList.contains('edit-minutes'))el.querySelector('.edit-seconds').focus();else saveInlineEditor(i)}
    });
  });
  render(i);
});
